...
/*char to hold the control signal from middleware*/
char option = '0';     


//led state use when blink 
int ledState = LOW;

 /*flag to indicate different comamnds*/
...
{ 
  ...
  while (1)
  { 
    if(flag_help)/*to make the RGB flag blink with shorter delay*/
    {
    ...
    if((!flag_pause)&&(!flag_help))  /*if pause is true, loop for nothing */
  {
    Wire.requestFrom(THERM, 2);
    Temperature_H = Wire.read();
    Temperature_L = Wire.read();
  ...
  /*p ->pause
  r ->resume
  c ->convert the unit between Fahrenheit and Celsius
  s ->help SOS feature*/ 
  if(option == 'p')
  {
    flag_pause = true;
...
void Dis_7SEG (int Decimal, byte High, byte Low, bool sign)
{
  ...
  //Serial.print(Decimal);  
    //Serial.print("before:");
    //Serial.print(High);
if(flag_help){                    /*send H,E,L,P to the 7 segment display*/
   Send7SEG (4,0x76);
    Send7SEG (3,0x79);
     Send7SEG (2,0x38);
      Send7SEG (1,0x73);
  ...
  if (Digit > 0)                 /* Display sign for unit if there is more space on 7-SEG */
  {
    if(flag_convert)
    {
      Send7SEG (Digit,0x71);        /*display F*/
    }
    else
    {
    Send7SEG (Digit,0x58);          /*display c*/
    }
    Digit--;
  }
...
void UpdateRGB (byte Temperature_H)
{
  ...
  if(flag_help)
  {
    // if the LED is off turn it on and vice-versa to make it blink:
    if (ledState == LOW)
      ledState = HIGH;
    else
      ledState = LOW;
